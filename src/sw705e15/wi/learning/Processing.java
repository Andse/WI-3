...
public class Processing
{
	...
	private DoubleMatrix stochasticGradientDescentStepForA(final UserMovieRatingMatrix data)
	{
		int randomMovieIndex = Random.nextInt(A.getRows());
		int randomUserIndex = Random.nextInt(B.getColumns());
	...
	private DoubleMatrix stochasticGradientDescentStepForB(final UserMovieRatingMatrix data)
	{
		int randomMovieIndex = Random.nextInt(A.getRows());
		int randomUserIndex = Random.nextInt(B.getColumns());
	...
	public HashMap<Integer, Double> prediction(final UserMovieRatingMatrix initialMatrix, final int userID, final List<Integer> movieIDs)
	{
		HashMap<Integer, Double> userMoviePredictions = new HashMap<>();
		final int userIndex = initialMatrix.userIDToIndexMapping.get(userID);
		...
		double userNormalization = (1.0 / observedUserRatings.get(userIndex)) * summarizedUserRatings.get(userIndex);
